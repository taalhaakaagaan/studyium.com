...
export const authAPI = {
    login: async (email: string, remember: boolean = true, portal: string = 'student') => {
        // @ts-ignore
        if (window.require) {
            // @ts-ignore
            const { ipcRenderer } = window.require('electron');
            return ipcRenderer.invoke('db:login', { email, remember, portal });
        }
        // Fallback for web dev (mock)
        console.warn("IPC not available. Mocking login.");
        return { success: true, requires2FA: true, email, role: portal === 'teacher' ? 'tutor' : 'student' };
    },
