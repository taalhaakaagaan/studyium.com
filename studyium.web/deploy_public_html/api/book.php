...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert Booking
    $stmt = $conn->prepare("INSERT INTO bookings (tutor_id, student_id, lesson_id, booking_date, status, payment, topic) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iiissds", $tutor_id, $student_id, $lesson_id, $booking_date, $status, $payment, $topic);
    
    if ($stmt->execute()) {
        echo json_encode(["success" => true, "message" => "Randevu oluşturuldu.", "id" => $stmt->insert_id]);
    } else {
