...
if __name__ == '__main__':
    zip_filename_base = 'deploy_20.zip'
    base_dir = os.getcwd()
    ...
    with zipfile.ZipFile(output_zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        print("Ensuring pdf.worker.min.js and .mjs are included...")
        for worker in ['pdf.worker.min.js', 'pdf.worker.min.mjs']:
            try:
                zipf.getinfo(worker)
                print(f"{worker} already in zip.")
            except KeyError:
                 worker_src = os.path.join(base_dir, 'public', worker)
                 if os.path.exists(worker_src):
                     zipf.write(worker_src, worker)
                     print(f"Added {worker} from public/")
                 else:
