...
function StudentDetailContent() {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            if (window.electron) {
                // @ts-ignore
                const userRes = await window.electron.invoke('db:get-user-details', studentId);
                if (userRes.success) {
                ...
                // Fetch Schedule
                // @ts-ignore
                const scheduleRes = await window.electron.invoke('db:get-schedule', { studentId });
                if (scheduleRes.success) {
                    const mappedLessons = scheduleRes.schedule.map((s: any) => ({
                        id: s.id,
                        day: s.day_of_week.substring(0, 3), // Convert Monday to Mon
    ...
    const handleSaveLesson = async () => {
        if (!selectedSlot || !topic || !user?.id) return;
        ...
        if (window.electron) {
            // @ts-ignore
            const res = await window.electron.invoke('db:save-schedule-item', payload);
            if (res.success) {
                const newLesson = {
                    id: res.id,
