...
type Tutor = {
    ...
    account_name?: string;
    iban?: string;
    active?: string; // 'active' or 'passive'
};
...
export default function AdminDashboard() {
    ...
    // Blog Editor State (adding missing state for consistency)
    const [blogFile, setBlogFile] = useState<File | null>(null);

    // Daily Question State
    const [dqDate, setDqDate] = useState(new Date().toISOString().split('T')[0]);
    const [dqCorrectAnswer, setDqCorrectAnswer] = useState("A");
    const [dqFile, setDqFile] = useState<File | null>(null);
    const [dqLoading, setDqLoading] = useState(false);
    const [dqMessage, setDqMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null);

    const handleDqFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            setDqFile(e.target.files[0]);
        }
    };
    ...
    const handleDqSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!dqFile) {
            setDqMessage({ type: 'error', text: 'Lütfen bir PDF dosyası seçin.' });
            return;
        }
        ...
        formData.append("question_date", dqDate);
        formData.append("correct_answer", dqCorrectAnswer);
        formData.append("pdf_file", dqFile);

        try {
            const res = await fetch("/api/admin/daily_question.php", {
                method: "POST",
                body: formData,
            });
            ...
            if (data.status === 'success') {
                setDqMessage({ type: 'success', text: 'Günün sorusu başarıyla yüklendi/güncellendi.' });
                setDqFile(null);
                // Reset file input visually if needed
                const fileInput = document.getElementById('dq-pdf-upload') as HTMLInputElement;
                if (fileInput) fileInput.value = '';
            } else {
                setDqMessage({ type: 'error', text: data.message || 'Bir hata oluştu.' });
            }
        } catch (error) {
            console.error(error);
            setDqMessage({ type: 'error', text: 'Sunucu hatası oluştu.' });
        } finally {
            setDqLoading(false);
        }
    };
    ...
    const toggleTutorStatus = async (tutor: Tutor) => {
        const newStatus = tutor.active === 'passive' ? 'active' : 'passive';
        try {
            const res = await fetch('/api/admin/edit_tutor.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: tutor.id, active: newStatus })
            });
            if (res.ok) {
                fetchTutors();
            } else {
        ...
        <div className="min-h-screen bg-background text-foreground flex flex-col">
            ...
            <div className="flex flex-1 relative">
                ...
                `}>
                    ...
                    <nav className="space-y-2">
                        ...
                        <div className="pt-4 border-t border-border mt-4">
                            <button onClick={() => { setActiveTab('daily_question'); setIsSidebarOpen(false); }}
                                className={`w-full text-left p-3 rounded-lg flex items-center gap-3 ${activeTab === 'daily_question' ? 'bg-primary text-primary-foreground' : 'hover:bg-accent text-primary font-bold'}`}>
                                <CalendarCheck className="h-5 w-5" /> Günün Sorusu
                            </button>
                        </div>
                    </nav>
                </aside>
                ...
                <main className="flex-1 p-4 md:p-8 overflow-auto w-full">
                                ...
                                <div className="md:col-span-2 space-y-4">
                                    ...
                                    {selectedNoteTopic ? (
                                        <div className="bg-card border border-border rounded-xl p-4 h-[600px] flex flex-col gap-4">
                                            ...
                                            <div className="flex-1 flex flex-col justify-center items-center border-2 border-dashed border-border rounded-xl bg-muted/20 relative hover:bg-muted/30 transition-colors">
                                                <input
                                                    type="file"
                                                    id="note-pdf-upload"
                                                ...
                                                <div className="flex flex-col items-center gap-2 pointer-events-none p-4 text-center">
                                                    {noteFile ? (
                                                        <>
                                                            ...
                                                            <span className="font-bold text-foreground">{noteFile.name}</span>
                                                            <span className="text-xs text-muted-foreground">{(noteFile.size / 1024).toFixed(1)} KB</span>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <Upload className="w-10 h-10 text-muted-foreground" />
                                                            <span className="text-muted-foreground font-medium">PDF Yükle</span>
                        ...
                        <div>
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (res.ok) {
                                    alert("Blog yazısı eklendi!");
                                    setNewBlog({ title: '', category: '', excerpt: '' });
                                    setBlogFile(null); // Reset file state
                                    if (fileInput) fileInput.value = '';
                                    fetchBlogs();
                                } else {
                                    alert("Hata oluştu.");
                                ...
                                <div className="border-2 border-dashed border-border rounded-xl p-6 bg-muted/20 text-center relative hover:bg-muted/30 transition-colors">
                                    <input
                                        type="file"
                                        id="blog-pdf-upload"
                                        accept="application/pdf"
                                    ...
                                    <div className="flex flex-col items-center gap-2 pointer-events-none">
                                        {blogFile ? (
                                            <>
                                                ...
                                                <span className="font-bold text-foreground">{blogFile.name}</span>
                                            </>
                                        ) : (
                                            <>
                                                <Upload className="h-8 w-8 text-muted-foreground" />
                                                <span className="font-semibold text-muted-foreground">Blog PDF Dosyası Seç</span>
                                            </>
                                        )}
                                    </div>
                                </div>
                    ...
                    {/* Daily Question Tab */}
                    {activeTab === 'daily_question' && (
                        <div>
                            <h2 className="text-3xl font-bold mb-8 flex items-center gap-3">
                                <CalendarCheck className="text-primary" />
                                Günün Sorusu Yönetimi
                            </h2>
                            <div className="bg-card border border-border rounded-2xl p-8 shadow-xl max-w-2xl">
                                <form onSubmit={handleDqSubmit} className="space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            type="date"
                                            value={dqDate}
                                            onChange={(e) => setDqDate(e.target.value)}
                                            className="w-full p-3 rounded-xl bg-background border border-input focus:border-primary outline-none transition-colors"
                                            required
                                        />
                                        <p className="text-xs text-muted-foreground">Sorunun yayınlanacağı tarih.</p>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex gap-4">
                                            {['A', 'B', 'C', 'D', 'E'].map(opt => (
                                                <label key={opt} className={`flex-1 cursor-pointer border rounded-xl p-3 flex items-center justify-center gap-2 transition-all ${dqCorrectAnswer === opt ? 'bg-primary text-primary-foreground border-primary' : 'bg-muted/20 border-border hover:bg-muted/50'}`}>
                                                    <input
                                                        type="radio"
                                                        name="correct_answer"
                                                        value={opt}
                                                        checked={dqCorrectAnswer === opt}
                                                        onChange={(e) => setDqCorrectAnswer(e.target.value)}
                                                        className="hidden"
                                                    />
                                    ...
                                    {/* PDF Upload */}
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium">Soru PDF Dosyası</label>
                                        <div className="border-2 border-dashed border-border rounded-xl p-8 text-center hover:bg-muted/10 transition-colors relative">
                                            ...
                                            <div className="flex flex-col items-center gap-2 pointer-events-none">
                                                {dqFile ? (
                                                    <>
                                                        <FileText className="w-10 h-10 text-primary" />
                                                        <span className="font-bold text-foreground">{dqFile.name}</span>
                                                        <span className="text-xs text-muted-foreground">{(dqFile.size / 1024).toFixed(1)} KB</span>
                                                    </>
                                                ) : (
                                                    <>
                                                        <Upload className="w-10 h-10 text-muted-foreground" />
                                                        <span className="text-muted-foreground">PDF dosyası seçmek için tıklayın veya sürükleyin</span>
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    {/* Status Message */}
                                    {dqMessage && (
                                        <div className={`p-4 rounded-xl flex items-center gap-3 ${dqMessage.type === 'success' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                                            {dqMessage.type === 'success' ? <CheckCircle size={20} /> : <AlertCircle size={20} />}
                                            {dqMessage.text}
                                        </div>
                                    )}
                                    ...
                                    <button
                                        type="submit"
                                        disabled={dqLoading}
                                        className="w-full bg-primary text-primary-foreground py-4 rounded-xl font-bold hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
                                    >
                                        {dqLoading ? 'Yükleniyor...' : 'Kaydet ve Yayınla'}
                                    </button>
                                        ...
                                        <tbody>
                                            {tutors.map(t => (
                                                <tr key={t.id} className="border-t border-border hover:bg-muted/50">
                                                    ...
                                                    <td className="p-4">
                                                        <button
                                                            onClick={() => toggleTutorStatus(t)}
                                                            className={`px-3 py-1 rounded-full text-xs font-bold transition-colors ${t.active === 'passive' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}
                                                        >
                                                            {t.active === 'passive' ? 'OFF' : 'ON'}
                                                        </button>
                                                    </td>
