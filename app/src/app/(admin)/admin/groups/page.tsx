...
export default function AdminGroupsPage() {
    ...
    useEffect(() => {
        const init = async () => {
            // No syncGroups here to avoid loop
    ...
    // Load Messages safely (no infinite loop)
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            ...
            if (res.success && isMounted) {
                ...
                // Only scroll if new messages? Or just first time?
                // scrollToBottom(); 
            }
        };
    ...
    }, [activeGroup?.id]); // Only change if ID changes
    ...
    return (
        <div className="flex h-[calc(100vh-100px)] border rounded-xl overflow-hidden shadow-sm bg-card">
            ...
            <div className="w-64 border-r bg-muted/20 flex flex-col">
                <div className="p-4 border-b font-semibold">Study Groups</div>
                                        ...
                                        <div className={`max-w-[70%] rounded-lg p-3 ${isMe ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                                            {!isMe && (
                                                <div className="flex items-center gap-1 mb-1">
                                                    ...
                                                    {msg.sender_role === 'teacher' && <span className="text-[10px] bg-indigo-500 text-white px-1 rounded">TEACHER</span>}
                                                </div>
                                            )}
